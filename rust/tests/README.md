...
- `05_env.sh` and `06_config.sh`: developer-friendly default secret passthrough/readable `.env`, explicit `--scramble`, short secret-named values, `--sec` implication, SSH agent/control-socket compatibility, descriptor-relative ancestor-symlink rejection and other fail-closed secret preparation, exact-byte project trust/invalidation, non-interactive bundled-profile materialization, directly selectable profiles, ordered recursive personal/work imports, profile/preset independence and precedence, home/project path anchoring, origin-aware grants, policy-file mutation/rename/link containment, and the best-effort command preflight's exact token/basename boundary.
...
- `14_input_validation.sh`: malformed CLI/configuration/imports, unsafe setup destinations, missing/escaping/linked/cyclic profile imports, non-UTF-8 filesystem-policy paths, and insecure custom-policy-parent failures before sandbox startup; each policy failure proves the child never executes.
...
- `18_builtin_commands.sh`: every documented built-in's status, representative output, and proof that dispatch never falls through to the sandbox; release acceptance reruns this focused suite against the installed prefix before uninstalling it.
