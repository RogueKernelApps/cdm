...
- `05_env.sh` and `06_config.sh`: developer-friendly default secret passthrough/readable `.env`, explicit `--scramble`, short secret-named values, `--sec` implication, descriptor-relative ancestor-symlink rejection and other fail-closed secret preparation, exact-byte project trust/invalidation, preset precedence, origin-aware grants, policy-file mutation/rename/link containment, and the best-effort command preflight's exact token/basename boundary.
