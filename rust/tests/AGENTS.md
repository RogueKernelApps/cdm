...
- Setup journeys must use a PTY and isolated `HOME`/`PATH`, prove detected entries and toggle/cancel behavior through the exact artifact, and inspect the resulting managed base/profile tree. Base-profile journeys that import `bundled/*.json` must prove profile provenance and missing optional harness state are preserved through the import graph.
