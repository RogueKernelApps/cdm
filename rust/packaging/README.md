...
./packaging/package.sh verify-runtime target/dist/cdm-0.1.6-aarch64-apple-darwin aarch64-apple-darwin
./packaging/package.sh verify target/dist/cdm-0.1.6-aarch64-apple-darwin aarch64-apple-darwin # redistributable completeness
```
...
The container installs GNU Wget rather than relying on the BusyBox applet because
...
codesigning identity from the imported keychain, allows noninteractive access to
that key only inside the disposable job keychain, verifies a timestamped probe signature
