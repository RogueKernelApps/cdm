...
pub trait HttpHandler: Clone + Send + Sync + 'static {
    ...
    /// Whether bytes with an unknown protocol prefix may fall back to an
    /// opaque CONNECT tunnel after interception was requested. Defaults to
    /// `true` for compatibility with upstream hudsucker behavior.
    fn should_tunnel_unknown_connect(
        &mut self,
        _ctx: &HttpContext,
        _req: &Request<Body>,
    ) -> impl Future<Output = bool> + Send {
