...
- `setup.rs` owns only terminal validation, fixed known-app detection without candidate execution, and catalog-ordered toggle selection. `config.rs` owns selected bundled files, recognizable managed `~/.cdm/base.json`, stale-known cleanup, and base-before-global runtime loading. Preserve user-owned/unknown files; never add an opaque registry, migration, or legacy schema.
