...
- `cdm setup` requires a terminal, structurally detects only fixed known apps without executing them, and presents an all-checked toggle menu. Keep its selected bundled files and transparent `~/.cdm/base.json` separate from user global policy. Do not add an opaque registry, migration layer, or legacy profile schema.
